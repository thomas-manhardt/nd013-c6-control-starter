 ...
 **/
 ...
 int main ()
 {
   ...
   **/
   ...
   PID pid_steer = PID();
   // steering PID gains tuned lower to reduce saturation; limits fixed at [1.2, -1.2]
   pid_steer.Init(0.12, 0.0, 0.02, 1.2, -1.2);

   PID pid_throttle = PID();
   // throttle PID gains: moderate Kp, tiny Ki, no derivative; limits fixed at [1.0, -1.0]
   pid_throttle.Init(0.25, 0.0005, 0.0, 1.0, -1.0);
   ...
   h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
   {
         ...
         if (s != "") {
           ...
           **/
           ...
           double error_steer;
           double steer_output;
           double desired_heading = yaw; // for debug output
 
           if (x_points.size() >= 2) {
             desired_heading = angle_between_points(x_points[0], y_points[0],
                                                           x_points[1], y_points[1]);
             double diff = desired_heading - yaw;
             while (diff > M_PI)  diff -= 2.0 * M_PI;
             while (diff < -M_PI) diff += 2.0 * M_PI;
             error_steer = diff;
           } else {
             error_steer = 0.0; // Not enough points to define heading
           }
           ...
           **/
           ...
           // Debug steering info
           cout << "[STEER_CTRL] step=" << i
                << " dt=" << new_delta_time
                << " yaw=" << yaw
                << " des_head=" << desired_heading
                << " err=" << error_steer
                << " steer_out=" << steer_output
                << endl;
           ...
           **/
           ...
           double desired_speed = v_points.empty() ? 0.0 : v_points.back(); // path planner target speed
           // NOTE: internal PID does steering_angle = -(P+I+D). To get positive output when we need acceleration,
           // we feed error as (actual - desired). Thus when actual < desired => negative error => -(P) positive control.
           error_throttle = velocity - desired_speed; // inverted sign so positive control => throttle
